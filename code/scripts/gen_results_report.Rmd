...
est_df <- read_delim(orss_gwas_resf,delim="\t")  %>%
  ...
  mutate(fgeneid=as.character(fgeneid),method=paste0("RSSp_",method))
...
# ```{r}
# filter(est_df,method=="RSSp_NoConfound",tbias==0) %>% ggplot(aes(x=abs(oracle_lnZ),y=abs(lnZ),col=method))+geom_point()+scale_x_log10()+scale_y_log10()+facet_grid(tpve~rel_bias,labeller="label_both",scales = "free")+geom_abline(slope=1,intercept=0)
# ```

```{r}
filter(est_df,method=="RSSp_NoConfound",tbias==0) %>% ggplot(aes(x=tpve,y=pve,col=method))+geom_point()++geom_abline(slope=1,intercept=0)
```



```{r}
mutate(comp_est_df,tgrp=paste0(tpve,method),t_rel_bias=round(tbias/tpve,3)) %>% ggplot(aes(x=method,y=abs(pve-tpve),group=tgrp,fill=method))+
  geom_boxplot()+
  facet_grid(tpve~t_rel_bias,labeller=label_both)+
  ylab(bquote(RMSE[PVE]))+
  xlab(bquote(PVE))
```
```{r}
library(ggjoy)
mutate(comp_est_df,t_rel_bias=round(tbias/tpve,3)) %>% ggplot(aes(x=abs(pve-tpve),y=method,fill=method))+geom_joy()+xlab(bquote(RMSE[PVE]))+facet_grid(tpve~t_rel_bias,labeller=label_both)
```
...
mutate(comp_est_df,t_rel_bias=round(tbias/tpve,3)) %>% ggplot(aes(x=tpve,y=pve))+
