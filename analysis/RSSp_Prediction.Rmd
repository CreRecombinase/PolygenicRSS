...
tpdf <- "/home/nwknoblauch/Desktop/scratch/polyg_scratch/RSSp_sim_gwas_pheno/scz2_trait.h5"
tparam_df <- est_df
...
ggplot(tparam_df,aes(x=factor(true_pve),y=cor_y_est_true))+geom_boxplot()+facet_grid(.~rel_bias,labeller = partial(label_both,sep=":\n"))+xlab(bquote(PVE))+ylab(bquote(rho(y[true]~','~y[posterior])))+ggtitle(bquote(Correlation~between~tilde(x)~bar(beta)[posterior]~and~tilde(x)~beta[true]))
```
```{r}
ggplot(y_df,aes(x=y_true,y_est))+geom_point()+facet_grid(true_pve~rel_bias,labeller=partial(label_both,sep=":\n"))+geom_smooth(method="lm")+xlab(bquote(y[true]))+ylab(bquote(y[posterior]))+ggtitle(bquote(Correlation~between~tilde(x)~beta[posterior]~and~tilde(x)~beta[true]))
```

```{r}
ggplot(tparam_df,aes(x=factor(true_pve),y=cor_y_oracle_true))+geom_boxplot()+facet_grid(.~rel_bias,labeller = partial(label_both,sep=":\n"))+xlab(bquote(PVE))+ylab(bquote(rho(y[true]~','~y[oracle])))+ggtitle(bquote(Correlation~between~tilde(x)~bar(beta)[oracle]~and~tilde(x)~beta[true]))
```
```{r}
...
```{r}
ggplot(tparam_df,aes(x=tsigu,y=true_sigu))+geom_point()+xlab(bquote(sigma[u][True]))+ylab(bquote(SD(beta[True])))+geom_abline(slope=1,intercept=0)+ggtitle("E[Var(B)]==Var(E[B])","When the  ")+scale_x_log10()+scale_y_log10()
```

```{r y_true_y_oracle,echo=F}
ggplot(y_df,aes(x=y_true,y=y_oracle))+geom_point()+geom_smooth(method="lm")+facet_grid(true_pve~rel_bias,labeller = partial(label_both,sep=":\n"))+geom_abline(slope=1,intercept=0)+xlab(bquote(X~beta[true]))+ylab(bquote(X~beta[oracle]))+ggtitle("RSSp prediction","Using true parameters for PVE and confounding")
```
...
```
```{r}
ggplot(tparam_df,aes(x=factor(true_pve),y=cor_beta_oracle_est))+geom_boxplot()+facet_grid(.~rel_bias,labeller = partial(label_both,sep=":\n"))+xlab(bquote(PVE))+ylab(bquote(rho(bar(beta)[oracle]~','~bar(beta)[posterior])))+ggtitle(bquote(Correlation~between~bar(beta)[posterior]~and~beta[oracle]))
```



```{r}
ggplot(y_df,aes(x=tsigu,y=sigu))+geom_point()+geom_abline(slope=1,intercept=0)+xlab(bquote(sigma[u]))+ylab(bquote(hat(sigma)[u]))+geom_smooth(method="lm")+facet_wrap(~rel_bias,labeller = partial(label_both,sep=":\n"))
```
