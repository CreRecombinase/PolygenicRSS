...
pve <- as.numeric(seq(0.1,0.3,length.out = 18))
...
gen_sim <- function(SNP,pve,bias=0,nreps,outdir,collapse_all=T,overwrite=F){
  ...
  bias_mat <- sapply(tparam_df$tbias,function(ta,p){rnorm(n=p,mean=0,sd=ta)},p=p)
...
outdir <- "../data/polygenic_sim_genotype2"
...
outdir <- "../data/polygenic_sim_genotype"
gen_sim(SNP,pve,bias,nreps =nreps,outdir = outdir,collapse_all = T)
```
