...
$$D^{\dagger\dagger}=\frac{1}{\sigma_u^2}I_p+ D^{\dagger^{-1}}=\sigma^2I_p+\text{diag}(\frac{1}{\lambda+c})$$
Simplifying further:
...
with $$D^{\dagger \dagger \dagger} = D^{\dagger \dagger^{-1}} D D^{\dagger^{-1}} =\text{diag}(\frac{\lambda}{c\sigma_u^2+\lambda \sigma_u^2+1})$$

Multiplying $U$ on the left by $Q^{T}$ yields a posterior density that can be computed in linear time:

$$Q^{T} U \sim N(D^{\dagger \dagger \dagger} Q^{T}\hat{U},D^{\dagger \dagger ^{-1}})$$
