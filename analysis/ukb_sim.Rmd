...
```{r,echo=F,message=F,warning=F}
ggplot(check_df,aes(x=true_h2,y=h2,col=method))+geom_point()+geom_smooth(method="lm",formula=y~x+0)+geom_abline(slope=1)+facet_wrap(~method)
```
...
ggplot(check_df,aes(x=method,y=abs(true_h2-h2),fill=method))+geom_boxplot()+geom_point()+facet_wrap(~true_h2,labeller="label_both")+ylab("RMSE")+xlab("Method")+theme(axis.text.x=element_text(angle = -90, hjust = 0))
