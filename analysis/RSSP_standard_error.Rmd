...
```{r}
ggplot(rssp_est,aes(x=sigu^2,y=bias))+geom_point()+geom_errorbar(aes(ymin=(bias-bias_var),ymax=(bias+bias_var))) +
...
```{r}
ggplot(rssp_est,aes(x=sigu_var,y=bias_var))+geom_point()+xlab(bquote(Var(hat(sigma)[u]^2)))+ylab(bquote(Var(hat(c))))
