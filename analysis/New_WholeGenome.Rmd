...
res_df <- map_df(res_d_rssp,read_delim,delim="\t") %>% filter(!is.na(simulation),!is.na(useLDshrink))  %>% filter(scenario=="VeryLargePVE",!is.na(doSplit))
...
res_df <- map_df(res_d_rssp,~read_delim(.x,delim="\t") %>% mutate(Intercept=as.character(Intercept))) %>% filter(!is.na(simulation),!is.na(useLDshrink)) %>% inner_join(tparam_df)%>% filter(scenario=="VeryLargePVE",!is.na(doSplit))
