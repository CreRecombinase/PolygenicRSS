...
```{r filenames,echo=F,message=F,warning=F,eval=F}
...
p <- ncol(SNP)
est_df <- estimate_RSSp_files(small_evdf,asimf,small_genof,chunksize = p,R_dataname = "shrink_R",doNoConfound = T,doLog=F) %>% mutate(doLog=F)
...
filter(est_df,method=="NoConfound",tbias==0) %>% ggplot(aes(x=tpve,y=pve))+geom_point()+geom_smooth(method="lm")+xlab(expression(paste("PVE")))+ylab(expression(hat("PVE")))+ggtitle("Estimate of PVE vs True PVE","No confounding")+geom_abline(intercept=0,slope=1)
```
...
```{r plot_pve_small}
filter(est_df) %>% ggplot(aes(x=tpve,y=pve))+geom_point()+geom_smooth(method="lm")+xlab(expression(paste("PVE")))+ylab(expression(hat("PVE")))+ggtitle("Estimate of PVE vs True PVE","No confounding")+geom_abline(intercept=0,slope=1)+facet_wrap(~method,labeller=label_both)
```


```{r}
library(ggjoy)
filter(est_df) %>% ggplot(aes(x=tbias/tpve,y=abs(pve-tpve)/tpve,col=method))+geom_point()+geom_smooth()+ggtitle("RMSE(PVE) with increasing Levels of confounding","Linear space vs Log space")+xlab("Confounding (Prop of PVE)")+ylab("RMSE(PVE)/PVE")
```
...
```{r}
filter(est_df,method=="Confound") %>% ggplot(aes(x=tbias/tsigu,y=a_hat/tsigu))+geom_point()+geom_abline(intercept=0,slope=1)+geom_smooth(method="lm")+ggtitle("Estimate of Counfounding vs Confounding")+xlab("True Confounding")+ylab("Estimate of Confounding")
```
...
filter(est_df_large,method=="NoConfound",tbias==0) %>% ggplot(aes(x=tpve,y=pve))+geom_point()+geom_smooth(method="lm")+xlab(expression(paste("PVE")))+ylab(expression(hat("PVE")))+ggtitle("Estimate of PVE vs True PVE","No confounding")+geom_abline(intercept=0,slope=1)
```

```{r}
filter(est_df_large) %>% ggplot(aes(x=tpve,y=pve,col=tbias))+geom_point()+geom_smooth(method="lm")+xlab(expression(paste("PVE")))+ylab(expression(hat("PVE")))+ggtitle("Estimate of PVE vs True PVE","No confounding")+geom_abline(intercept=0,slope=1)+facet_wrap(~method)
```
...
```{r}
# library(ggjoy)
filter(est_df_large) %>% ggplot(aes(x=tbias/(tpve),y=abs(pve-tpve)/tpve,col=method))+geom_point()+geom_smooth()+ggtitle("RMSE(PVE)/PVE with increasing Levels of confounding")+xlab("Confounding (as a proportion of PVE)")+ylab("RMSE(PVE)/PVE")+scale_y_log10()+facet_wrap(~method)
...
```{r}
filter(est_df_large,method=="Confound") %>% ggplot(aes(x=tbias,y=a_hat))+geom_point()+geom_abline(intercept=0,slope=1)+geom_smooth(method="lm")+ggtitle("Estimate of Counfounding vs Confounding")+xlab("True Confounding")+ylab("Estimate of Confounding")
...
dest_df_large <- estimate_RSSp_files(large_evdf,ndir_simf,large_genof,chunksize = p,R_dataname = "R",doNoConfound = T,doLog=F) %>% mutate(doLog=F)
saveRDS(dest_df_large,"../output/polygenic_sim_direct.RDS")
...
```{r}
dest_df_large <- readRDS("../output/polygenic_sim_direct.RDS")
...
```{r}
filter(dest_df_large,method=="Confound") %>% ggplot(aes(x=tbias,y=a_hat))+geom_point()+geom_abline(intercept=0,slope=1)+geom_smooth(method="lm")+ggtitle("Estimate of Counfounding vs Confounding")+xlab("True Confounding")+ylab("Estimate of Confounding")
```
