...
//[[Rcpp::export]]
double norm_allfun_cpp(const mapa par,const mapa dvec, const mapa quh){
  const double sigu=exp(par(0));
  const double varu=sigu*sigu;
  const double a=exp(par(1));
  double tsum = ((dvec*dvec*varu+dvec+a).log()).sum();
  double tprod = ((quh*(1/(dvec*dvec*varu+dvec+a)))*(quh)).sum();
  return -(-0.5*(tsum+tprod));
}
...
evd_alnorm <- function(Q,rd,uh,sigu_bounds=c(0.3,2.6),a_bounds=c(0,1)){
  # revd <- eigen(R)
  # rd <- revd$values
  # Q <- revd$vectors
  quh <- c(t(Q)%*%uh)
  ...
  maxb <- c(sigu_bounds[2],
            a_bounds[2])
  par0 <- runif(2,min=minb,max = maxb)
  ldat  <- optim(par0,fn=norm_allfun_cpp,dvec=rd,quh=quh)
  return(data_frame(sigu=exp(ldat$par[1]),a=exp(ldat$par[2]),lnZ=ldat$value))
}
...
estimation_func <- function(Q,rd,asimd,parallel=F,confound_bounds = c(0,1)){
  ...
  res_df <- foreach(i=1:ng,.export=c('evd_norm','evd_anorm','mutate'),.packages=c("dplyr"),.combine=bind_rows) %do%{
    ...
    bind_rows(
      ...
      evd_alnorm(Q,rd,tuh_b,sigu_bounds=sigu_bounds,a_bounds = confound_bounds) %>% mutate(fgeneid=i,method="Confound",pve=p/n*sigu^2)
    )
  }
  tresults <- inner_join(res_df,tparamdf)
  return(tresults)
}
...
est_param <- function(evdf,simf,genof){
  if(!file.exists(evdf)){
    R <- read_2d_mat_h5(genof,groupname = "/",dataname = "R")
    evdR <- eigen(R)
    Q <- evdR$vectors
    rd <- evdR$values
  ...
  return(estimation_func(Q,rd,asimd,parallel = F))
}
...
tparamdf <- select(res_df,fgeneid,tpve,tbias,tsigu) %>% distinct()
conf_comp_small <- select(res_df,fgeneid,sigu,a,pve,method,tpve) %>% mutate(rmse=abs(pve-tpve),rel_rmse=rmse/tpve) %>% select(-tpve) %>%  gather(key = param,value=value,a,pve,sigu,rmse,rel_rmse) %>% unite(method_param,method,param,sep = "_") %>% spread(key = method_param,value) %>% inner_join(tparamdf) %>% select(-No_Confound_a)

mutate(conf_comp_small,rmse_rat=Confound_rmse/No_Confound_rmse) %>% rename(a=tbias) %>% ggplot(aes(x=(a^2),y=log(rmse_rat)))+geom_point()+geom_smooth(method="lm")+geom_abline(slope=0,intercept=0)+xlab("Confounding")+ylab("log(RMSE(No_Confound)/RMSE(Confound))")+ggtitle("Estimator with Confounding Outperforms","With increasing level confounding")
...
```{r}
mutate(conf_comp_small,rmse_rat=Confound_rmse-No_Confound_rmse,tbias=tbias^2) %>% rename(a=tbias) %>% filter(a>0) %>%  ggplot(aes(x=a/tpve,y=rmse_rat))+geom_point()+geom_smooth(method="lm")+geom_abline(slope=0,intercept=0)+xlab("Confounding/PVE")+ylab("RMSE(No_Confound)-RMSE(Confound)")+facet_wrap(~a)
...
# Larger data set, Larger Simulation


```{r larger_sim}
#res_df_larger <- est_param(large_evdf,large_simf,largest_simf)
res_df_larger <-readRDS("../data/polygenic_sim_genotype_larger/results.RDS")
#trds <- readRDS(largest_simf)
#ttparamdf <- trds$tparam_df
```


```{r plot_large}
filter(res_df_larger,method=="No_Confound",tbias==0) %>% ggplot(aes(x=tpve,y=(pve-tpve)^2/tpve))+geom_point()+geom_smooth(method="lm")+xlab(expression(paste("PVE")))+ylab(expression(hat("PVE")))+ggtitle("Estimate of PVE vs True PVE")
```

```{r plot2_large}
rename(res_df_larger,ahat=a) %>% rename(a=tbias) %>% ggplot(aes(y=sqrt((tpve-pve)^2)/tpve,x=method,fill=method))+geom_boxplot()+xlab(expression(paste("Estimator")))+
  ylab("RMSE(PVE)")+
  ggtitle("Estimating PVE With Confounding",
          paste0("Using a model with and without a confounding term"))+facet_wrap(~a)+scale_y_sqrt()
```

```{r}
filter(res_df_larger,method=="Confound") %>% rename(c_hat=a) %>% rename(c=tbias) %>% ggplot(aes(x=c,y=sqrt(c_hat)))+geom_point()
```
